...
__device__ void d_reduce_sum(volatile sc * shmem_vals, sc * add_result_to) {
...
__device__ void d_triangles_to_geometry_000(
...
__device__ sc d_sl_kernel_do_anti_tau_anti_t_regular_in_time_regular_in_space(
  ...
  //  ttau > 0, norm > 0
  sc value = ( ttau / ( 4.0 * kp.pi * kp.alpha * norm )
               + norm / ( 8.0 * kp.pi * kp.alpha_2 ) )
      * erf( norm / ( 2.0 * sqrt_d * kp.sqrt_alpha ) )
    + sqrt_d / ( 4.0 * kp.pi * kp.alpha * kp.sqrt_pi * kp.sqrt_alpha )
      * exp( -( norm * norm ) / ( 4.0 * ttau * kp.alpha ) );
...
__global__ void d_apply_regular_sl_p0_p0(const sc * x, lo ld_x, sc * y_perm, lo ld_y_perm, sc alpha) {
  ...
  lo &blocks = c_mesh.n_temporal_elements; // number of blocks of matrix, not gpu threadblocks
  lo tid = threadIdx.x;
  ...
  for (lo i_trl = threadIdx.x; i_trl < n_elems; i_trl += blockDim.x) {
    ...
    sc val_prev = 0; // delta=0 and 0s wiil be done by the cpu
    sc val_curr = 0;
    sc val_next = 0;
    ...
    lo max_block = c_mesh.n_temporal_elements;
    for (lo block = 0; block < max_block; block++) {
      shmem_y_vals[tid] = alpha * matrix_val * x[ ld_x * block + col ];
      __syncthreads();
      d_reduce_sum(shmem_y_vals, &y_perm[row * ld_y_perm + block]);
    }
    ...
    for (lo diag = 1; diag < blocks; diag++) {
      ...
      for (lo block = 0; block < max_block; block++) {
        ...
        lo block_col = block;
        shmem_y_vals[tid] = alpha * matrix_val * x[ ld_x * block_col + col ];
        __syncthreads();
        d_reduce_sum(shmem_y_vals, &y_perm[row * ld_y_perm + block_row]);
      }
    }
...
#pragma omp master
    { // cuda computations preparation and start
      ...
      d_apply_regular_sl_p0_p0<<< gridSize, blockSize, shmemSize >>>(d_x, ld_x, d_y_perm, ld_y_perm, alpha);
