classdef kernel_heat_sl_2 < kernel & matlab.mixin.Copyable
  ...
  methods    
    function obj = kernel_heat_sl_2( alpha )
      obj.alpha = alpha;
      obj.ht = 0;
      obj.d = 0;
    end
    ...
    function value = eval_repr( obj, x, y, ~ )
      norm = sqrt( ( x - y ).^2 * [ 1; 1; 1 ] );     
      value = G_anti_tau( obj, norm, ( obj.d - 1 ) * obj.ht ) ...
          - G_anti_tau( obj, norm, obj.d * obj.ht );
    end
  end
  
  methods (Access = protected)
    function cp = copyElement( obj )
      cp = kernel_heat_sl_2( obj.alpha );
      cp.ht = obj.ht;
      cp.d = obj.d;
    end
    ...
    %%%%% int G dtau      
    %%%%% delta > 0 && norm > 0
    function res = G_anti_tau_regular( obj, norm, delta )
      res = erf( norm / sqrt( 4 * obj.alpha * delta ) ) ...
        ./ ( 4 * pi * obj.alpha * norm );
    end
