...
For $V_0(\vr)$ we obtain in a similar fashion
\begin{align*}
  V_0(\vr) &= \sqrt{\frac{h_t}{\alpha^3}} \int_{0}^{1} G^{\dif\hat \tau}_1\bigg(\frac{\vr}{\sqrt{\alpha h_t}},0\bigg) - G^{\dif\hat \tau}_1\bigg(\frac{\vr}{\sqrt{\alpha h_t}},\hat t\bigg) \,\dif\hat t \\
...
\begin{equation*}
  ...
  \begin{cases}
  \displaystyle
    \alpha \vn_\vy \cdot \nabla_\vy \int_{0}^{t_1} \int_{0}^{t} G_\alpha(\vr,t-\tau) \,\dif\tau \,\dif t & \text{for } d = 0, \\[4mm]
  \displaystyle
    \alpha \vn_\vy \cdot \nabla_\vy \int_{t_d}^{t_{d+1}} \int_{0}^{t_1} G_\alpha(\vr,t-\tau) \,\dif\tau \,\dif t & \text{for } d \in \{1,\ldots,E_t-1\}.
  \end{cases}
\end{equation*}
For $d > 0$ we can use the substitution \eqref{eq:subst_tau_t} and the scaling condition~\eqref{eq:G_scaling} to obtain
\begin{align*}
  K^d(\vr) = 
\end{align*}
