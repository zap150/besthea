...
\begin{equation*}
  W u (\vx,t) := \int_{\partial\Omega} \alpha\frac{\partial G_\alpha}{\partial \vn_\vy}(\vx-\vy,t-\tau) u(\vy,\tau) \,\dif\vs_\vy \,\dif\tau,
\end{equation*}
...
\begin{align*}
  \langle Ku_h, q \rangle_\Sigma &= \sum_{i=1}^{k-1} \sum_{j=1}^{N_\vx} u_{i,j} \int_{\gamma_\ell} \int_{\partial\Omega} \varphi^1_{\vx,j}(\vy) \alpha \int_{t_{d+1}}^{t_d} \int_{0}^{t_1} \frac{\partial G_\alpha}{\partial \vn_\vy}(\vx-\vy,t-\tau) \,\dif\tau \,\dif t \,\dif\vs_\vy \,\dif\vs_\vx \\
...
\end{align*}
Since $G^{\dif\hat\tau\dif\hat t}_1$ only depends on the norm of its first argument, we easily obtain
\begin{equation}
\label{eq:dn_G_dt_dtau}
  \frac{\partial G^{\dif\hat\tau\dif\hat t}_1}{\partial \vn_\vy} = \frac{\partial G^{\dif\hat\tau\dif\hat t}_1}{\partial \|\tilde \vr\|} \vn_\vy \cdot \nabla_\vy \frac{\|\vx-\vy\|}{\sqrt{\alpha h_t}} = -\frac{1}{\sqrt{\alpha h_t}} \frac{\partial G^{\dif\hat\tau\dif\hat t}_1}{\partial \|\tilde \vr\|} \frac{(\vx-\vy)\cdot \vn_\vy}{\|\vx-\vy\|} = -\frac{1}{\sqrt{\alpha h_t}} \frac{\partial G^{\dif\hat\tau\dif\hat t}_1}{\partial \|\tilde \vr\|} \frac{\tilde \vr \cdot \vn_\vy}{\|\tilde \vr\|}
\end{equation}
with
\begin{equation*}
...
\end{equation*}
However, when integrating over two identical triangles it holds $\tilde \vr \cdot \vn_\vy = 0$ almost everywhere. \todo{what about neighbouring elements? Need SS even for $\delta>0$?} Conversely, for $\|\tilde \vr\|>0$ we have
\begin{equation*}
...
\begin{align*}
  ...
  &- \sum_{i=1}^{k-1} \sum_{j=1}^{N_\vx} u_{i,j} \int_{\partial\Omega} \int_{\partial\Omega} n(\vx) \cdot n(\vy) \varphi^1_{\vx,\ell}(\vx) \varphi^1_{\vx,j}(\vy) \alpha^2 \int_{t_d}^{t_{d+1}} \int_{t_0}^{t_1} \frac{\partial G_\alpha}{\partial \tau}(\vx-\vy,t-\tau) \,\dif\tau \,\dif t \,\dif\vs_\vy \,\dif\vs_\vx \\
    &- \sum_{j=1}^{N_\vx} u_{k,j} \int_{\partial\Omega} \int_{\partial\Omega} n(\vx) \cdot n(\vy) \varphi^1_{\vx,\ell}(\vx) \varphi^1_{\vx,j}(\vy) \alpha^2 \int_{t_d}^{t_{d+1}} \int_{t_0}^{t} \frac{\partial G_\alpha}{\partial \tau}(\vx-\vy,t-\tau) \,\dif\tau \,\dif t \,\dif\vs_\vy \,\dif\vs_\vx.
\end{align*}
Since the surface curls of spatially piecewise linear functions are piecewise constant, we can rewrite on of the summands in the first part as
\begin{align*}
...
To evaluate the discretized representation formula in the temporal nodes $t_k = k h_t$ we have to compute the contribution of the single-layer potential
\begin{equation*}
