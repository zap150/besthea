...
\subsection{Locally essential cluster trees}
The locally essential time tree of a process is the subtree of the full time cluster tree which contains all clusters $I$ that satisfy at least one of the following three conditions:
\begin{itemize}
  \item The process is responsible for $I$.
  \item $I$ is in the interaction list of a cluster for which the process is responsible.
  \item $I$ has to be visited on a path from the root to one of the clusters in the local essential tree.
\end{itemize}
Similarly the locally essential space-time cluster tree is defined as a subtree of the full space-time cluster tree.

\subsection{Some ideas on generating the space-time cluster tree}
...
Instead of using the original recursive routine, switch to a level-wise tree generation to reduce communication. 
\begin{enumerate}
...
\subsection{The locally essential time cluster tree}
The FMM algorithm is based on a traversal of the local time cluster tree by every process. The mesh generator provides the processes only with the initial time cluster tree, which needs to be extended if the depth of the locally essential space-time cluster tree of a process is greater. This can be done by projection: For every cluster in the locally essential space-time cluster tree, add its temporal component as a time-cluster to the initial time cluster tree.
...
algorithm which is based on a grouping of operations via time clusters. A process executes operations only for time clusters for which it is responsible. These clusters are collected in appropriate lists, which are used to control the execution. Dependencies are checked on the level of the time clusters and handled with explicit flags. Priorities are handled by the order of the clusters in the lists. 
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \While{ the lists are not empty }
      \State Check for received data and update dependency flags;
      ...
      \State \texttt{delete} = \Call{executeOperations}{\texttt{next\_cluster}, \texttt{list\_index}};
...
A pseudocode for the function \textproc{executeOperations} is given in Alg.\ref{alg:fmm_execute_operations}. It is responsible for executing the appropriate primary computation operations, i.e.~functions which trigger the grouped execution of the computations in the space-time cluster tree. Furthermore it starts the sending operations and is responsible for updating the dependency flags inside of a process. More details about the primary computation operations are given in the next section. In particular, we discuss how the space-time clusters associated with a given time cluster can be identified.

\begin{algorithm}
  \caption{The execution of the primary operations} \label{alg:fmm_execute_operations}
  \begin{algorithmic}[1]
    \Function{executeOperations}{time cluster \texttt{I}, \texttt{list\_index}}
      \If{\texttt{list\_index} == 0}
        \State Call \Call{callS2MOperations}{\texttt{I}};
        \For{\texttt{J} such that \texttt{I} $\in \mathcal{I}(\texttt{J})$} \Comment{$\mathcal{I}$ denotes the interaction list}
          \State Call \Call{sendMomentsForM2L}{\texttt{I}, \texttt{J}};
          \State Set M2L dependency flag;
        \EndFor
        \State Call \Call{callM2MOperations}{\texttt{I}};
        \State Call \Call{sendProcessedMoments}{\texttt{I}, $\parent(\texttt{I})$}; \label{algline:send_proc_moments}
        \State Set M2M dependency flag;
        \State \Return \texttt{true}
      \ElsIf{\texttt{list\_index} == 1}
        \State Call \Call{callL2LOperations}{\texttt{I}};
        \State Set L2L completion flag;
        \If{all M2L and L2L operations are completed}
          \State Call \Call{callL2TOperations}{\texttt{I}};
          \For{$\texttt{I}_{\mathrm{c}} \in \children(\texttt{I})$}
            \State Call \Call{sendParentalLocals}{\texttt{I}, $\texttt{I}_{\mathrm{c}}$};
            \State Set L2L dependency flag;
          \EndFor
        \EndIf
        \State \Return \texttt{true}
      \ElsIf{\texttt{list\_index} == 2}
        \For{\texttt{J} $\in \mathcal{I}(\texttt{I})$}
          \If{M2L for \texttt{J} is ready but not completed}
            \State Call \Call{callM2LOperations}{\texttt{J}, \texttt{I}};
      ...
      \ElsIf{\texttt{list\_index} == 3}
        \State Call \Call{callNearfieldOperations}{\texttt{I}};
        \State \Return \texttt{true}
      \EndIf
    \EndFunction
  \end{algorithmic}
...
\subsection{The primary computation operations} \label{sec:primary_operations}
The primary computation operations in the time tree are responsible for creating appropriate secondary operations in the space-time cluster tree $\mathcal{T}_{s,t}$. More precisely, a primary computation operation for a time cluster $I$ creates secondary operations for all associated space-time clusters $\{Z_j\}_j$ in the space-time cluster tree with $Z_j = I \times X_j$. These clusters $\{Z_j\}_j$ can be determined either by a suitable mapping, or by traversing the space-time cluster tree at every call. Having a proper mapping allows to create secondary operations in an OpenMP taskloop or parallel for construct, which allows to parallely execute similarly sized chunks of operations avoiding too small task granularity. On the other hand, traversing the cluster tree is closer to the original implementation and does not introduce additional structures. It can be parallelized with OpenMP tasks, which can be grouped. However in case of a recursive tree traversal the grouping of tasks can in general be unbalanced.
...
In Alg.~\ref{alg:s2m_and_m2m_traversal} and~\ref{alg:s2m_and_m2m_mapping} we sketch implementations of the primary task operations \textproc{callS2MOperations} and \textproc{callM2MOperations}. Alg.~\ref{alg:s2m_and_m2m_traversal} is based on a tree traversal while Alg.~\ref{alg:s2m_and_m2m_mapping} relies on a mapping between time clusters and their associated space-time clusters. The routines \textproc{executeS2M} and \textproc{executeGroupedM2M} are the same in both algorithms. The routine \textproc{executeS2M} coincides with the S2M routine in the non-parallel pFMM version, which computes the moment contributions for the space-time elements in a given space-time cluster for a given source vector. The routine \textproc{executeGroupedM2M}($Z_j^p$, \texttt{configuration}) executes all M2M operations for the parent cluster $Z_j^p$ and all of its left or right children in time according to the given configuration. The M2M operations per se are the same as in the non-parallel pFMM version. In Alg.~\ref{alg:s2m_and_m2m_traversal} we use OpenMP tasks which we group using the final clause, while in Alg.~\ref{alg:s2m_and_m2m_mapping} we use OpenMP tasks which we generate using the taskloop construct.
